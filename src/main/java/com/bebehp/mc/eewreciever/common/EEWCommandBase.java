	...
	public void execute(final MinecraftServer server, final ICommandSender icommandsender, final String[] astring) throws CommandException {
		if (astring.length>=1&&(StringUtils.equalsIgnoreCase(astring[0], "p2p")||StringUtils.equalsIgnoreCase(astring[0], "p"))) {
						...
						if (EEWRecieverMod.accessToken==null) {
							...
							try {
								ChatUtil.sendPlayerChat(icommandsender, ChatUtil.byText("Twitterと連携設定をし、Pinコードを入手して下さい(クリックでURLを開く)")
										.setStyle(new Style().setColor(TextFormatting.GOLD).setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, this.tweetQuakeSetup.getAuthURL()))));
							} catch (final TwitterException e) {
								Reference.logger.error(e.getStatusCode());
								ChatUtil.sendPlayerChat(icommandsender, ChatUtil.byText("URLの生成に失敗しました").setStyle(new Style().setColor(TextFormatting.RED)));
							...
							ChatUtil.sendPlayerChat(icommandsender, ChatUtil.byText("/eew setup pin <Pin>でコードを入力して下さい(クリックで提案)")
									.setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/eewreciever setup pin "))));
						} else {
			...
			if (QuakeMain.INSTANCE.getStatus()) {
				...
				ChatUtil.sendPlayerChat(icommandsender, ChatUtil.byText("EEWRecieverはすでに無効です。").setStyle(new Style().setColor(TextFormatting.RED)));
			}
